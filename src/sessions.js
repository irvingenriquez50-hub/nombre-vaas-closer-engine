...
export async function startSession(userId) {
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // WhatsApp a veces manda el remoteJid en formato "@lid" (identificador nuevo) en vez
      // del número de teléfono normal — eso no coincide con el jid que guardamos cuando se
      // agregó el número. senderPn siempre trae el número de teléfono real, así que lo usamos
      // como el identificador verdadero del lead.
      const jid = rawJid.endsWith("@lid") && msg.key.senderPn ? msg.key.senderPn : rawJid;
      if (jid !== rawJid) console.log(`🔀 remoteJid era @lid (${rawJid}), usando senderPn en su lugar: ${jid}`);
