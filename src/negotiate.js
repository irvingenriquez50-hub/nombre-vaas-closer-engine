...
function decide(prevState, r, tiers, { forceAccept = false } = {}) {
  ...
  // REGLA DE ORO: solo se cierra con un número REAL que la marca haya puesto sobre
  // la mesa. Nunca se inventa un precio. Si no hay ninguna oferta suya guardada
  // (por ejemplo en conversaciones viejas de antes de este sistema), NO se cierra
  // nada — se deja para que Irving lo revise a mano.
  if (forceAccept) {
    const realOffer = next.theirBest ?? next.theirLast ?? their ?? null;
    if (realOffer == null) {
      return { action: "no_action", reason: "sin oferta real de la marca", state: next };
    }
    return { action: "close", price: realOffer, videos, state: next };
  }
...
export async function getNextMove(conversation, tiers, negState = null, opts = {}) {
  ...
  // No hay nada que decir ni que cerrar — no se genera ningún mensaje.
  if (decision.action === "no_action") {
    console.warn(`⏹️  Sin acción (${decision.reason || "sin motivo"}) — no se manda nada ni se cierra nada.`);
    return {
      replyText: null,
      noAction: true,
      closed: false,
