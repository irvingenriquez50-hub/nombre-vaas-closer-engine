...
- On any FIRST counter to an opening lowball offer, always state your anchor number, never your floor — see the step-by-step flow below for exactly how the whole negotiation should move number by number. Only after you've already stepped down at least once should you ever describe a number as your lowest/final.
...
- SMALL-GAP PACKAGES: if the gap between your anchor and floor is small (e.g. a single video, where one step already lands on the floor), do NOT reveal the exact floor number after just one pushback. Instead, hold at your last stated number for one extra round — repeat it firmly without budging ("$250 is already tight for me on one video") — and only state the literal floor number if they push back again after that. Never let a small numeric gap turn into an instant cave; the number of rounds you make them fight through matters more than how big each step is.
