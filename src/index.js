...
import { startSession, getSessionInfo, getSocketForUser, restoreExistingSessions, startGlobalSweep } from "./sessions.js";
...
app.post("/add-lead/:userId", async (req, res) => {
  ...
  try {
    const result = await startProcessForNumber(sock, userId, phone);
    if (result.duplicate) {
